...
import { CallToolRequestSchema, ListToolsRequestSchema, ListResourcesRequestSchema, ListResourceTemplatesRequestSchema, ListPromptsRequestSchema, GetPromptRequestSchema, ReadResourceRequestSchema, } from "@modelcontextprotocol/sdk/types.js";
...
export async function ensureStagehand() {
    if (stagehandConfig.env === "LOCAL" &&
        ...
        throw new Error('Using a local browser without providing a CDP URL is not supported. Please provide a CDP URL using the LOCAL_CDP_URL environment variable.\n\nTo launch your browser in "debug", see our documentation.\n\nhttps://docs.stagehand.dev/examples/customize_browser#use-your-personal-browser');
    }
    try {
        if (!stagehand) {
...
export function createServer() {
    ...
    server.setRequestHandler(CallToolRequestSchema, async (request) => {
        try {
            ...
            try {
                ...
                return {
                    content: [
                        {
                            type: "text",
                            text: `Failed to initialize Stagehand: ${errorMsg}.\n\nConfig: ${JSON.stringify(stagehandConfig, null, 2)}`,
                        },
                        {
                            type: "text",
                            text: `Operation logs:\n${operationLogs.join("\n")}`,
            ...
            const result = await handleToolCall(request.params.name, request.params.arguments ?? {}, stagehand);
