...
- **Interaction:** The main tool for interacting with the page is `stagehand_act`. It can be used in two ways:
    - **Action-based:** Use `action` (and optionally `variables`) to perform actions based on natural language descriptions (e.g., "Click the login button"). Use this method when you are sure that the action is clear and the element is easily identifiable on the page.
